...
class HybridFrameAnalyzer:
    ...
    def check_frame_relevance_with_llm(self, sentence: str, frame_name: str, hypothesis: str,
                                       frame_definition: str) -> Tuple[bool, float, str]:
        """
        Determine if the frame is relevant to the sentence using LLM
        Returns a tuple of (is_relevant, confidence_score, llm_response)
        """
        try:
...
"""
            ...
            # Return the full LLM response along with the relevance and confidence
            return is_relevant, confidence, output
        ...
        try:
            ...
            for result in results:
                specialized_data.append({
                    ...
                    'LLM_Reasoning': result.get('LLM_Response', '')  # Full LLM response
                })
    ...
    def process_dataset(self, input_csv: str, output_csv: str):
        ...
        try:
            ...
            for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing rows", position=0, leave=True):
                ...
                is_relevant, confidence, llm_response = self.check_frame_relevance_with_llm(
                    row['Sentence'],
                    frame_name,
                    hypothesis,
            ...
            if predictions and ground_truths:
                ...
                metrics_row = pd.DataFrame([{
                    ...
                    'Frame_Accepted': '',
                    'Ground_Truth': '',
                    'LLM_Response': '',  # Include empty LLM response in metrics row
